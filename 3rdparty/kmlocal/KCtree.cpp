...
//	for the point indices.  Normally, this will be nullptr (meaning
    ...
    KMdatIdxArray	pi)		// point indices (optional)
{
    ...
    if (pa == nullptr) {			// no points supplied?
    ...
    if (bb_lo == nullptr || bb_hi == nullptr) // boundng box fully specified?
	kmEnclRect(pa, pidx, n, dd, bnd_box);	// no, construct from points
					// save bounding box
    if (bb_lo != nullptr)			// if lower point given, then use it
    	bnd_box.lo = kmAllocCopyPt(dd, bb_lo);

    if (bb_hi != nullptr)			// same for upper point
    ...
    bnd_box(dd)				// create initial bounding box
{
    ...
    skeletonTree(pa, n, dd, n_max, bb_lo, bb_hi, nullptr);
...
//	If the leaf cell has no point, then nullptr is returned.
...
{  return (n_data == 1 ? kcPoints[bkt[0]] : nullptr);  }
...
{
    assert(sum != nullptr);			// should already be allocated
    int n_child = 0;				// n_data of child
    KMpoint s_child = nullptr;			// sum of child
...
//	to nullptr after deleting them, which keeps the KMkd_split
...
{
    if (sum != nullptr) kmDeallocPt(sum);	// deallocate sum
}
    ...
    double*	 	sqDist)			// sq'd distance to center
{
	...
	if (closeCtr != nullptr) closeCtr[bkt[i]] = cands[minK];
	if (sqDist != nullptr) sqDist[bkt[i]] = minDist;
    }
}
