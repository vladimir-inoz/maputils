...
//	times sqrt(d).
//----------------------------------------------------------------------
static KMpointArray cgClusters = nullptr;	// cluster storage
...
{
    if (cgClusters == nullptr || new_clust) {// need new cluster centers
	if (cgClusters != nullptr)		// clusters already exist
	...
	int		max_dim)	// maximum dimension of the flats
{
    ...
    static KMpointArray control = nullptr;	// control vectors

    if (control == nullptr || new_clust) {		// need new cluster centers
	if (control != nullptr) {			// clusters already exist
...
void kmClusEllipsoids(		// clustered around ellipsoids
	...
	int		max_dim)	// maximum dimension of the flats
{
    static KMpointArray clusters = nullptr;	// cluster centers
    static KMpointArray stdDev = nullptr;		// standard deviations

    if (clusters == nullptr || new_clust) {	// need new cluster centers
	if (clusters != nullptr)			// clusters already exist
	    kmDeallocPts(clusters);		// get rid of them
	if (stdDev != nullptr)			// std deviations already exist
